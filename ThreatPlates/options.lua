...
-- Titulo de la ventana de opciones: solo el nombre, sin numero de
-- version. Antes se leia del .toc y quedaba un "v6.5.0" que es de la
-- base original (Threat Plates), no de esta build.
local ADDON_NAME = ...
TidyPlatesThreat.title = TidyPlatesThreat.DISPLAY_NAME
local path = "Interface\\Addons\\Nidhaus_Plates\\ThreatPlates\\Media\\Artwork\\"
local db
...
-- Simplified build: el dialogo "TPTP Discord" solo lo abria el boton de la
-- pestana About, que ya no existe. Se elimina para no dejarlo colgando.
...
MediaRegister("statusbar", "ThreatPlatesBar", [[Interface\Addons\Nidhaus_Plates\ThreatPlates\Media\Artwork\TP_BarTexture.tga]])
MediaRegister("font", "Accidental Presidency", [[Interface\Addons\Nidhaus_Plates\ThreatPlates\Media\Fonts\Accidental Presidency.ttf]])
-- Extra fonts
MediaRegister("font", "Continuum Medium", [[Interface\Addons\Nidhaus_Plates\ThreatPlates\Media\Fonts\ContinuumMedium.ttf]])
MediaRegister("font", "Domyouji Regular", [[Interface\Addons\Nidhaus_Plates\ThreatPlates\Media\Fonts\DomyoujiRegular.ttf]])
...
-- ============================================================
-- Marcador de sanadores (modulo Healers)
--
-- Sus ajustes NO viven en el perfil de AceDB como el resto de la pestaña,
-- sino en la global BGHsettings, que es como los guardaba el addon del que
-- viene. Se dejan ahi a proposito: asi el modulo sigue siendo el mismo
-- codigo de siempre y se puede actualizar sin reescribirle la base de datos.
-- De ahi que necesite sus propios get/set en vez de GetValue/SetValue.
--
-- Ademas guarda los booleanos como 1 y 0, no como true/false, asi que las
-- casillas necesitan un par aparte que traduzca.
-- ============================================================
local function BGHmodule()
	return _G.NidhausPlatesHealers
end

local function BGHready()
	return BGHsettings ~= nil
end

local function GetBGHflag(info)
	return BGHready() and BGHsettings[info.arg] == 1
end

local function SetBGHflag(info, val)
	if not BGHready() then return end
	local key = info.arg
	if key == "enabled" then
		local m = BGHmodule()
		if m and m.SetEnabled then m.SetEnabled(val and true or false) return end
	end
	BGHsettings[key] = val and 1 or 0
	local m = BGHmodule()
	if m and m.ApplySettings then m.ApplySettings() end
end

local function GetBGH(info)
	return BGHready() and BGHsettings[info.arg]
end

local function SetBGH(info, val)
	if not BGHready() then return end
	BGHsettings[info.arg] = val
	local m = BGHmodule()
	if m and m.ApplySettings then m.ApplySettings() end
end

-- Todo lo que no sea la casilla maestra se apaga cuando el modulo esta off.
local function BGHoff()
	return not (BGHready() and BGHsettings.enabled == 1)
end
...
-- Setter propio para el icono clicable del totem.
--
-- No sirve SetThemeValue: espera un arg de tres niveles (db[a][b][c]) y este
-- ajuste vive suelto en la raiz del perfil, asi que b y c llegaban nil y
-- reventaba al indexar.
--
-- Ademas hay que recalcular el hitbox a mano: los estilos lo construyen una
-- sola vez en ADDON_LOADED, asi que sin esto el cambio no se veria hasta
...
local function GetOptions()
			...
			args = {
								...
								Hiding = {
									...
									args = {
										...
										HideSnakes = {
											...
											order = 5,
											width = "double",
											name = L["Hide Hunter Snakes"],
											desc = L["Hides the nameplates of Viper and Venomous Snake from the hunter's Snake Trap."],
											descStyle = "inline",
											get = GetValue,
											set = SetValue,
											arg = {"hideSnakes"}
										...
										HideSnakesFriendly = {
											...
											order = 6,
											width = "double",
											name = L["Hide Your Own Snakes"],
											desc = L["Also hides the snakes summoned by you and your allies, not just the enemy ones."],
											descStyle = "inline",
											get = GetValue,
											set = SetValue,
										...
										HideMirrorImage = {
											...
											order = 7,
											width = "double",
											name = L["Hide Mirror Images"],
											desc = L["Hides the nameplates of the mage's Mirror Image copies."],
											descStyle = "inline",
											get = GetValue,
											set = SetValue,
											arg = {"hideMirrorImage"}
							...
							args = {
								...
								Coloring = {
									...
									args = {
										...
										-- Simplified build: 'Custom HP Color' and 'Raid Mark HP Color' option groups removed.
										ThreatColors = {
											...
											inline = true,
											args = {
												-- Simplified build: "Show Threat Glow" toggle removed (glow is off).
												Header = {
													name = L["Colors"],
													type = "header",
													order = 2
										...
										Texture = {
											...
											args = {
												Preview = {
													...
													type = "execute",
													order = 1,
													image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\EliteArtWidget\\" .. db.settings.eliteicon.theme
												},
												Style = {
													type = "select",
													order = 2,
													...
													set = function(info, val)
														...
														db[a][b][c] = val
														for i = 1, #themeList do
															TidyPlatesThemeList["Threat Plates"][themeList[i]][b]["texture"] = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\EliteArtWidget\\" .. val
														end
														options.args.NameplateSettings.args.EliteIcon.args.Options.args.Texture.args.Preview.image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\EliteArtWidget\\" .. val
														Update()
													end,
						...
						BGHealers = {
							...
							args = {
								Enable = {
									...
									args = {
										Enable = {
											name = L["Enable Healer Marks"],
											type = "toggle",
											desc = L["Marks the nameplates of enemy and allied healers in battlegrounds."],
											descStyle = "inline",
											width = "double",
											order = 1,
											get = GetBGHflag,
								...
								Detection = {
									...
									args = {
										CLEUtracking = {
											name = L["Combat Log"],
											type = "toggle",
											desc = L["Detects a healer as soon as they cast one of about ninety telltale healing spells."],
											descStyle = "inline",
											width = "double",
											order = 1,
											get = GetBGHflag, set = SetBGHflag, arg = "CLEUtracking"
										},
										CLEUfix = {
											name = L["Combat Log Fix"],
											type = "toggle",
											desc = L["Keeps the combat log from going quiet during long battlegrounds."],
											descStyle = "inline",
											width = "double",
											order = 2,
											get = GetBGHflag, set = SetBGHflag, arg = "CLEUfix"
										},
										WSSFtracking = {
											name = L["Scoreboard"],
											type = "toggle",
											desc = L["Detects a healer by comparing healing against damage on the battleground scoreboard."],
											descStyle = "inline",
											width = "double",
											order = 3,
											get = GetBGHflag, set = SetBGHflag, arg = "WSSFtracking"
										},
										h2dRatio = {
											name = L["Healing to Damage Ratio"],
											type = "range",
											order = 4,
											min = 1, max = 5, step = 0.1, isPercent = false,
											disabled = function() return BGHoff() or BGHsettings.WSSFtracking ~= 1 end,
											get = GetBGH, set = SetBGH, arg = "h2dRatio"
										},
										healingThreshold = {
											name = L["Healing Threshold"],
											type = "range",
											order = 5,
											min = 10000, max = 100000, step = 5000,
											disabled = function() return BGHoff() or BGHsettings.WSSFtracking ~= 1 end,
											get = GetBGH, set = SetBGH, arg = "healingThreshold"
										}
									}
								},
								Icon = {
									...
									args = {
										iconStyle = {
											...
											type = "select",
											order = 1,
											values = { Blizzlike = L["Blizzlike"], Minimalist = L["Minimalist"] },
											get = GetBGH, set = SetBGH, arg = "iconStyle"
										},
										iconAnchor = {
											name = L["Anchor"],
											type = "select",
											order = 2,
											values = { left = L["Left"], top = L["Top"], right = L["Right"] },
											get = GetBGH, set = SetBGH, arg = "iconAnchor"
										},
										iconSize = {
											name = L["Size"],
											type = "range",
											...
											get = GetBGH, set = SetBGH, arg = "iconSize"
										},
										iconXoffset = {
											name = L["X Offset"],
											type = "range",
										...
										iconYoffset = {
											...
											order = 5,
											min = -40, max = 40, step = 1,
											get = GetBGH, set = SetBGH, arg = "iconYoffset"
										},
										iconInvertColor = {
											name = L["Invert Colors"],
											type = "toggle",
											desc = L["By default enemies are red and allies are blue."],
											descStyle = "inline",
											width = "double",
											order = 6,
											get = GetBGHflag, set = SetBGHflag, arg = "iconInvertColor"
										}
									}
								}
							}
				...
				ThreatOptions = {
					...
					args = {
								...
								Options = {
									...
									args = {
										PrevLow = {
											...
											order = 1,
											width = "double",
											image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\ThreatWidget\\" .. db.threat.art.theme .. "\\" .. "HIGH",
											imageWidth = 256,
											imageHeight = 64
										},
										PrevMed = {
											...
											order = 2,
											width = "double",
											image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\ThreatWidget\\" .. db.threat.art.theme .. "\\" .. "MEDIUM",
											imageWidth = 256,
											imageHeight = 64
										},
										PrevHigh = {
											...
											order = 3,
											width = "double",
											image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\ThreatWidget\\" .. db.threat.art.theme .. "\\" .. "LOW",
											imageWidth = 256,
											imageHeight = 64
										},
										Style = {
											...
											args = {
												Dropdown = {
													...
													set = function(info, val)
														...
														local p = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\ThreatWidget\\"
						...
						-- Simplified build: "Dual Spec Roles" removed (addon is fixed to DPS/Healing).
					}
				},
				Widgets = {
					name = L["Widgets"],
						...
						ArenaPartyIDWidget = {
							...
							args = {
								Enable = {
									...
									args = {
										Enable = {
											name = L["Show Arena / Party ID"],
											type = "toggle",
											desc = L["Shows the arena number (1-5) or the party number (1-4) on the nameplate."],
											descStyle = "inline",
											width = "double",
											order = 1,
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "ON"}
										},
										ShowArena = {
											name = L["Arena Opponents"],
											type = "toggle",
											width = "double",
											order = 2,
											disabled = function() return not db.arenaPartyIDWidget.ON end,
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "showArena"}
										},
										ShowParty = {
											...
											order = 3,
											disabled = function() return not db.arenaPartyIDWidget.ON end,
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "showParty"}
										}
									}
								},
								Appearance = {
									...
									args = {
										Font = {
											...
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "font"}
										},
										Size = {
											name = L["Size"],
											type = "range",
											order = 2,
											min = 6, max = 36, step = 1,
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "size"}
										},
										Outline = {
											name = L["Outline"],
											type = "select",
											...
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "outline"}
										},
										Anchor = {
											name = L["Anchor"],
											type = "select",
											order = 4,
											values = { Left = L["Left"], Center = L["Center"], Right = L["Right"], Top = L["Top"], Bottom = L["Bottom"] },
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "anchor"}
										},
										OffsetX = {
											name = L["Offset X"],
											type = "range",
											...
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "x"}
										},
										OffsetY = {
											name = L["Offset Y"],
											type = "range",
											order = 6,
											min = -50, max = 50, step = 1,
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "y"}
										},
										UseClassColor = {
											...
											width = "double",
											order = 7,
											get = GetValue, set = SetValue, arg = {"arenaPartyIDWidget", "useClassColor"}
										},
										Color = {
											name = L["Color"],
											type = "color",
											order = 8,
											disabled = function() return db.arenaPartyIDWidget.useClassColor end,
											get = function() local c = db.arenaPartyIDWidget.color return c.r, c.g, c.b end,
											set = function(_, r, g, b)
												local c = db.arenaPartyIDWidget.color
												c.r, c.g, c.b = r, g, b
												Update()
											end
										}
				...
				-- Simplified build: pestana "About" eliminada.
			}
		}
	end
	...
	local ClassOpts = {
		Style = {
			...
			set = function(info, val)
				...
				for k_c, v_c in pairs(CLASS_SORT_ORDER) do
					options.args.Widgets.args.ClassIconWidget.args.Textures.args["Prev" .. k_c].image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\ClassIconWidget\\" .. db.classWidget.theme .. "\\" .. CLASS_SORT_ORDER[k_c]
				end
			end,
			-- "circle": juego circular con anillo dorado, tomado de
			-- RefinedBlizzPlates (de Khal). Mismos 10 nombres de archivo
			-- que los otros temas, asi que el widget no necesita cambios.
			values = {default = "Default", transparent = "Transparent", circle = "Circle"},
			arg = {"classWidget", "theme"}
		},
		-- Borde del icono. La FORMA no se elige: se saca del tema, porque un
		-- marco cuadrado sobre el icono redondo queda mal y al reves tambien.
		Border = {
			name = L["Icon Border"],
			order = -2,
			type = "toggle",
			width = "double",
			desc = L["Draws a border around the class icon. Square on the square themes, a ring on Circle."],
			descStyle = "inline",
			get = GetValue,
			set = SetValue,
			arg = {"classWidget", "border"}
		},
		BorderThickness = {
			...
			disabled = function() return not db.classWidget.border or db.classWidget.theme == "circle" end,
			get = GetValue,
			set = SetValue,
			arg = {"classWidget", "borderThickness"}
		},
		BorderColor = {
			name = L["Border Color"],
			order = -4,
			type = "color",
			desc = L["Color of the border on units that are not your target."],
			disabled = function() return not db.classWidget.border end,
			get = function()
				local c = db.classWidget.borderColor
				return c.r, c.g, c.b
			end,
			set = function(_, r, g, b)
				local c = db.classWidget.borderColor
		...
		BorderColorTarget = {
			name = L["Border Color (Target)"],
			order = -5,
			type = "color",
			desc = L["Color of the border on your current target."],
			disabled = function() return not db.classWidget.border end,
			get = function()
				local c = db.classWidget.borderColorTarget
				return c.r, c.g, c.b
			end,
			set = function(_, r, g, b)
				local c = db.classWidget.borderColorTarget
	...
	for k_c, v_c in pairs(CLASS_SORT_ORDER) do
		ClassOpts["Prev" .. k_c] = {
			...
			image = "Interface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\ClassIconWidget\\" .. db.classWidget.theme .. "\\" .. CLASS_SORT_ORDER[k_c]
		}
				...
				Textures = {
					...
					args = {
						Icon = {
							...
							width = "double",
							order = 0,
							image = "Interface\\Addons\\Nidhaus_Plates\\ThreatPlates\\Widgets\\TotemIconWidget\\" .. db.totemSettings[totemID[k_c][2]][7] .. "\\" .. totemID[k_c][2]
						},
						Style = {
							name = "",
							type = "select",
							...
							set = function(info, val)
								SetValue(info, val)
								options.args.Totems.args[totemName].args.Textures.args.Icon.image = "Interface\\Addons\\Nidhaus_Plates\\ThreatPlates\\Widgets\\TotemIconWidget\\" .. db.totemSettings[totemID[k_c][2]][7] .. "\\" .. totemID[k_c][2]
							end,
						...
						TargetButton = {
							...
							func = function()
								if UnitExists("Target") then
									...
									if db.verbose then
										print(TidyPlatesThreat.DISPLAY_NAME .. ": " .. L["No target found."])
									end
								end
							end
						},
						...
						Copy = {
							...
							func = function()
								...
								if type(clipboard) == "table" and db.verbose then
									print(TidyPlatesThreat.DISPLAY_NAME .. ": " .. L["Copied!"])
								end
							end
						},
						Paste = {
							...
							func = function()
								if type(clipboard) == "table" then
									...
									if db.verbose then
										print(TidyPlatesThreat.DISPLAY_NAME .. ": " .. L["Pasted!"])
									end
								else
									if db.verbose then
										print(TidyPlatesThreat.DISPLAY_NAME .. ": " .. L["Nothing to paste!"])
									end
								end
	...
	do
		...
		for i = 0, 72 do
			arrows["arrow" .. i] = "|TInterface\\AddOns\\Nidhaus_Plates\\ThreatPlates\\Widgets\\TargetArtWidget\\arrow" .. i .. ".tga:45:45|t"
		end
	end
		...
		intoptions = {
			...
			args = {
				...
				openoptions = {
					type = "execute",
					name = L["Open Config"],
					-- SIN LOGO: la imagen era el titulo "Threat Plates" de
					-- la base original, dibujado como textura. Queda solo
					-- el boton con su texto.
					width = "double",
					func = function()
				...
				-- Simplified build: settings merged in from the old "Tidy Plates" panel.
				coreheader = {
					type = "header",
					name = L["Nameplate Settings"],
					order = 30
				...
				castwatcher = {
					type = "toggle",
					name = L["Show Non-Target Casting Bars (When Possible)"],
					width = "double",
					order = 40,
					get = function() return TidyPlates:GetCastWatcher() end,
					set = function(_, val) TidyPlates:SetCastWatcher(val) end
				},
				minimapicon = {
					type = "toggle",
					...
					width = "double",
					order = 50,
					get = function() return TidyPlates:GetMinimapButton() end,
					set = function(_, val) TidyPlates:SetMinimapButton(val) end
				},
				blizzoptions = {
					type = "execute",
					name = L["Blizzard Nameplate Motion & Visibility"],
					width = "double",
					order = 60,
					func = function() TidyPlates:OpenBlizzardNameplateOptions() end
				},
				resetconfig = {
					type = "execute",
					name = L["Reset Configuration"],
					desc = L["Restores the Tidy Plates defaults and reloads the UI."],
					width = "double",
					order = 70,
					confirm = true,
					func = function() TidyPlates:ResetConfiguration() end
				}
			}
		}
	end
...
function TidyPlatesThreat:SetUpInitialOptions()
	...
	-- Simplified build: single entry in the Interface > AddOns list.
	-- El nombre visible tambien pasa a Nidhaus_Plates: era lo unico que
	-- seguia diciendo "Tidy Plates" en la lista de Interfaz.
	self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions(
		"Tidy Plates: Threat Plates Dialog", TidyPlatesThreat.DISPLAY_NAME)
end

-- Simplified build: TidyPlatesThreat:AddOptions() removed (Auras/Stances tab).
...
function TidyPlatesThreat:SetUpOptions()
	...
	-- Simplified build: Auras / Stances / Shapeshifts / Presences tab removed.
